			...
			if (response.status === 201) {
				...
				alert(
					`Error ${response.status}: ${
						response.statusText
					}\n${txt.slice(12, txt.length - 2)/* Error message from server */}`
				);
			}
		} catch (e) {
							...
							<div className="grid grid-cols-1">
								...
								<div className="block p-1.5">
									<label
										...
										className="text-gray-700"
									>
										Repeat Password <span className={(password&&(password!=password2))? "text-red-400 text-sm" : "hidden"}>Passwords don't match</span>
									</label>
									<input
                                        required
										type="password"
